...
int main(int argc, const char **argv) {
  ...
  try {
    ...
    bool VERBOSE = false;
    string chrom_name = "chr1";
    size_t chrom_size = 250000000;
    ...
    opt_parse.add_opt("output", 'o', "Name of output file (default: stdout)", 
		      false , outfile);
    opt_parse.add_opt("nreads", 'n', "number of reads to simulate (approximate)", 
		      false , n_reads);
    opt_parse.add_opt("mu_low", 'm', "expected reads per site in low read-count bases",
		      false , mu_lowreads);
    opt_parse.add_opt("mu_high", 'M', "expected reads per site in high read-count bases",
		      false , mu_highreads);
    opt_parse.add_opt("sigma_low", 's', "standard deviation in low read-count bases",
		      false , sigma_lowreads);
    opt_parse.add_opt("sigma_high", 'S', "standard deviation in high read-count bases",
		      false , sigma_highreads);
