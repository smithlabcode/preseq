/* Copyright (C) 2020-2025 Masaru Nakajima and Andrew D. Smith
 *
...
static inline auto
bam_is_rev(const bam1_t *b) -> bool {
  return (b->core.flag & BAM_FREVERSE) != 0;
}

static inline auto
bam_get_qname(const bam1_t *b) -> char * {
  return reinterpret_cast<char *>(b->data);  // NOLINT(*-reinterpret-cast)
}

static inline auto
bam_get_cigar(const bam1_t *b) -> std::uint32_t * {
  // NOLINTNEXTLINE(*-reinterpret-cast)
  return reinterpret_cast<std::uint32_t *>(b->data + b->core.l_qname);
}

static inline auto
bam_get_seq(const bam1_t *b) -> std::uint8_t * {
  // start of data +  bytes for query/read name+ bytes for cigar
  return b->data + b->core.l_qname + (b->core.n_cigar << 2);
}

static inline auto
bam_get_qual(const bam1_t *b) -> std::uint8_t * {
...
static inline auto
bam_get_aux(const bam1_t *b) -> std::uint8_t * {
  return b->data + b->core.l_qname + (b->core.n_cigar << 2) +
...
static auto
sam_realloc_bam_data(bam1_t *b, std::size_t desired) -> int {
  // NOLINTBEGIN(*-no-malloc,*-owning-memory)
  std::uint32_t new_m_data = desired;
  roundup_to_power_of_2(new_m_data);
  if (new_m_data < desired) {
    // (from sam.c) Not strictly true but we can't store the size
    errno = ENOMEM;
    return -1;
  }
...
bam_set1_core(bam1_core_t &core, const std::size_t l_qname,
              const std::uint16_t flag, const std::int32_t tid,
              const hts_pos_t pos, const std::uint8_t mapq,
              const std::size_t n_cigar, const std::int32_t mtid,
  ...
  static constexpr int min_shift = 14;
  static constexpr int n_lvls = 5;
  core.pos = pos;
  core.tid = tid;
...
bam_set1_wrapper(bam1_t *bam, const std::size_t l_qname, const char *qname,
                 ...
                 const std::int32_t mtid, const hts_pos_t mpos,
                 const hts_pos_t isize, const std::size_t l_seq,
                 const std::size_t l_aux) -> int {
  ...
  const std::size_t data_len =
    (l_qname + qname_nuls + n_cigar * sizeof(std::uint32_t) + (l_seq + 1) / 2 +
     l_seq);
...
static inline auto
to_insertion(const std::uint32_t x) -> std::uint32_t {
...
static inline auto
to_softclip(const std::uint32_t x) -> std::uint32_t {
...
static inline auto
merge_cigar_ops(const std::size_t n_cigar,
                std::uint32_t *cigar) -> std::size_t {
  if (n_cigar < 2)
    return n_cigar;
  auto c_itr1 = cigar;
...
static inline auto
correct_cigar(bam1_t *b) -> std::size_t {
...
static inline auto
get_rlen(const bam1_t *b) -> hts_pos_t {  // less tedious
  return bam_cigar2rlen(b->core.n_cigar, bam_get_cigar(b));
}

static inline auto
get_l_qseq(const bam1_t *b) -> std::size_t {
...
get_full_and_partial_ops(const std::uint32_t *cig_in,
                         ...
                         std::uint32_t *partial_oplen) -> std::uint32_t {
 ...
 * It is assumed that the input std::uint8_t value is of form "xx" or "x-",
 ...
 */
constexpr auto byte_revcomp_table = std::array<std::uint8_t, 256>{
  // clang-format off
...
merge_by_byte(bam1_t const *const a, bam1_t const *const b, bam1_t *const c) {
  ...
  if (is_a_odd) {
    ...
    //                      or [ aa aa aa a- ]
    c_seq[a_num_bytes - 1] &= 0xf0;  // NOLINT(*-avoid-magic-numbers)
    c_seq[a_num_bytes - 1] |=
...
are_mates(const bam1_t *one, const bam1_t *two) -> bool {
...
static inline auto
truncate_overlap(const bam1_t *a, const std::uint32_t overlap,
                 bam1_t *c) -> int {
  ...
  const std::int64_t nm =
    bam_aux2i(bam_aux_get(a, "NM"));  // ADS: do better here!
...
merge_overlap(const bamxx::bam_rec &a, const bamxx::bam_rec &b,
              const std::uint32_t head, bamxx::bam_rec &c) -> int {
  if (c.b == nullptr)
    c.b = bam_init1();
...
merge_non_overlap(const bamxx::bam_rec &a, const bamxx::bam_rec &b,
                  const std::uint32_t spacer, bamxx::bam_rec &c) -> int {
  if (c.b == nullptr)
    c.b = bam_init1();
...
static inline auto
keep_better_end(const bam1_t *a, const bam1_t *b, bam1_t *c) -> int {
...
apply_cigar(const bamxx::bam_rec &aln, std::string &to_inflate,
  ...
  const std::size_t orig_len = to_inflate.length();
  if (i != orig_len)
...
// NOLINTEND(*-pointer-arithmetic,*-narrowing-conversions,*-constant-array-index)
