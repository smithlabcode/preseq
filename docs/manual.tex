...
\usepackage{times,fullpage,graphicx,amsmath, cite, alltt, enumitem, titlesec, verbatim}
...
\section{Screening multiple libraries}
\label{sec:multlib}
...
\begin{figure}[h!]
\centering
\centering{\includegraphics[totalheight = 0.5\textheight, trim=0cm 0cm 0cm 2cm, clip = true]{/Users/Vicky/Desktop/WORK/InitialExperimentComplexityCurves_copy.pdf}}
\end{figure}
...
\begin{figure}[h!]
\centering
\centering{\includegraphics[totalheight = 0.5\textheight, trim=0cm 0cm 0cm 2cm, clip = true]{/Users/Vicky/Desktop/WORK/FullExperiment_copy.pdf}}
\end{figure}
...
\begin{comment}
Here are three different examples that help show what would be considered a relatively poor library and a relatively good library, as well as compare the complexity curves obtained from running \fn{c\_curve} and \fn{lc\_extrap}. The black diagonal line represents an ideal library, in which every read is a distinct read. A full experiment is down sampled at 5\% to obtain a mock initial experiment, as shown here, where we have the complexity curves obtained by \fn{c\_curve} of the initial experiments:
~\newline
\newline
\begin{figure}[h!]
\centering
\centering{\includegraphics[totalheight = 0.5\textheight, trim=0cm 2cm 0cm 2cm, clip = true]{/Users/Vicky/Desktop/WORK/InitialExperimentComplexityCurves_copy.pdf}}
\end{figure}

Notice how closely the red and blue curves follow each other. A deviation will come in the full experiment, as seen in the next figure. 
\newpage
This is a comparison of the full experiment complexity curves and the extrapolated complexity curves created using information from the initial experiments above as input. The dashed lines indicate the complexity curves predicted by \fn{lc\_extrap}, and the solid lines are the expected complexity curves of the full experiments, obtained using \fn{c\_curve}.  Using this, it is clear that the green lines represent a poor library, while the red and blue lines are relatively good. Note that the dashed curves follow the solid curves very closely, only differing slightly towards the end. 
\newline
\newline
\begin{figure}[h!]
\centering
\centering{\includegraphics[totalheight = 0.5\textheight, trim=0cm 2.8cm 0cm 2cm, clip = true]{/Users/Vicky/Desktop/WORK/FullExperiment_copy.pdf}}
\end{figure}
\end{comment}
